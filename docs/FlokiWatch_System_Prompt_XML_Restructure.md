# FlokiWatch Agent System Prompt — XML Restructure Design

## Current State
- 481 lines of Markdown with ## headers
- Long continuous text where instructions get lost
- Agent ignores some instructions (Fibonacci, EMA200 were ignored until we added reinforcement)

## New Structure
The entire system prompt will be wrapped in XML tags that clearly delineate each section. Claude processes these as distinct semantic blocks.
...
### ADDED (new):
- `<trade_continuity>` now includes SELF-QUESTIONING AFTER LOSSES section
- Clear XML boundaries for every section

### REMOVED / CONDENSED:
- Verbose examples section (was ~80 lines) — condensed to essential examples in setup_evaluation
- Redundant repetition of "indicators don't veto" (appeared 3+ times)
- Markdown headers replaced with XML tags
- "REMEMBER" section merged into philosophy
- Duplicate output format explanations consolidated

### RESTRUCTURED:
- Every section has a clear XML tag name
- Order follows importance: identity → continuity → philosophy → expertise → analysis → output → calibration → rules
- Final reminder stays at the very end (last thing Claude reads)

## Section Order Rationale
1. identity + role — WHO you are (sets the persona)
2. trade_continuity — CHECK MEMORY FIRST (most important for consistency)
3. philosophy + gold_expertise — HOW you think
4. brain_context — How to interpret Brain data
5. analysis_method — HOW to read the chart (Structure → Macro → Indicators)
6. setup_evaluation — HOW to evaluate quality
7. risk_rules — Non-negotiable limits
8. decisions — WHAT you can decide
9. output_format — HOW to format the response
10. confidence_calibration — HOW to set confidence
11. momentum_rules + data_quality + calendar — Specific knowledge
12. reject_format — Special format for REJECT
13. final_reminder — JSON only (LAST thing read)

## Implementation Notes
- The prompt is significantly shorter (~350 lines vs 481)
- XML tags replace Markdown headers
- Self-questioning after losses is integrated into trade_continuity
- Examples are condensed (the Agent doesn't need 8 full examples — it needs clear rules)
- The REJECT section with memory context is preserved but cleaned up

## Validation Plan
After implementation:
1. Restart bot, wait for proactive snapshot
2. Verify Agent produces valid JSON
3. Verify Agent mentions Fibonacci, EMA200, swings, price changes
4. Verify Agent self-questions if repeating a losing trade direction
5. Compare reasoning quality with pre-XML prompt
