...
- **Rex monitor (FLO-211):** Runs 4 tools every 30 min (divergence, correlation, regime, session). No LLM — deterministic classifier. Writes `data/rex_monitor.json`. Floki pulls via `get_rex_monitor`. Simba wakes Floki on CRITICAL (2h debounce). Bull/Bear debate injection of monitor findings was removed (downstream consumer decoupled in FLO-243 — Bull/Bear output no longer reaches Floki, so injecting monitor findings into Bull/Bear was dead weight).
