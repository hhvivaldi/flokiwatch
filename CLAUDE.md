...
## ICT zones bridge (FLO-455 Phase 1, CEO 2026-05-21)

Additive ICT overlay for the MT5 chart, mirroring the S/R bridge — **does NOT touch the S/R system**. Zero AI cost. **Phase 1 (done):** new module `ict_zones.py` writes UNMITIGATED H1 Fair Value Gaps + liquidity sweeps to `ict_zones.json` in `MQL5\Files\` (`config.ICT_ZONES_JSON_PATH`). Called fail-soft from `central_brain.analyze_with_brain()` each cycle (try/except pass — never blocks the Brain result). Schema: `{timestamp, zones:[{type:FVG, direction:bullish|bearish, timeframe:"H1", top, bottom, status:"unmitigated", candle_time} | {type:SWEEP, direction:high|low, level, candle_time}]}`. Tests: `test_flo455_ict.py` (pure mapping + fail-soft + atomic JSON).
  - **DATA SOURCE — Floki's own detectors, NOT the package (CEO 2026-05-22).** First cut used `smartmoneyconcepts==0.0.27`, but it returns 0 zones at gold-scale prices ($4500+) — not calibrated for gold. Dropped (removed from requirements; not imported). `ict_zones.py` now reuses `agent_tools._scan_fvgs` / `_scan_sweeps` (FLO-438), which work on gold and already return only unfilled FVGs. Lazy-imports `agent_tools` inside the function to avoid the `central_brain → ict_zones → agent_tools` cycle. **No Order Blocks** — Floki's tools detect FVGs + sweeps only (OB can be added later if an OB detector is built). BSL→`direction:"high"`, SSL→`"low"`.
  - **Phase 2 (NOT done):** `ICTZoneDrawer.mq5` reads `ict_zones.json` every 10s, draws FVG rectangles + sweep dashed lines (mirror `SRZoneDrawer.mq5`); operator MT5 compile/attach. **Phase 3 (NOT done):** `get_ict_zones` tool. Validate zone quality visually before building the EA. Linear: FLO-455 (In Progress).
