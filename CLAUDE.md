...
- **Daily loss limit gate (FLO-439, CEO 2026-05-17).** Validator rejects new plans when today's realized P&L is at or below -2.0% of account balance. Threshold `LIMIT_PCT = 2.0` is hardcoded in `_check_daily_loss_limit` (snow/validator.py). Account snapshot supplied via the new `author_account` kwarg on `validate_plan`: `{"balance": float, "today_pnl_usd": float}`. `agent_tools.submit_plan_to_snow` pulls balance from `executor.get_account_info()` and today's realized P&L from history.db (sum of `profit` where `close_time >= today_00:00_UTC`) via helper `_today_realized_pnl_usd`. Fail-soft on missing account info, parse errors, or zero balance (logs `DAILY_LOSS_LIMIT_DEGRADED` WARN). Rationale per the research doc: the worst losing days are the ones that should have stopped at -2% but ended at -6%; prevents revenge trading after a bad sequence.
