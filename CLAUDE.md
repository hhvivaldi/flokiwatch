...
Additive ICT Smart Money Concepts overlay for the MT5 chart, mirroring the S/R bridge — **does NOT touch the S/R system**. Zero AI cost (pure Python, `smartmoneyconcepts==0.0.27`). **Phase 1 (done):** new module `ict_zones.py` detects UNMITIGATED H1 Order Blocks, Fair Value Gaps, and liquidity sweeps (`smc.swing_highs_lows` → `smc.fvg`/`smc.ob`/`smc.liquidity`; unmitigated = `MitigatedIndex.isna()`) and writes `ict_zones.json` to `MQL5\Files\` (`config.ICT_ZONES_JSON_PATH`). Called fail-soft from `central_brain.analyze_with_brain()` each cycle (try/except pass — never blocks the Brain result). Schema: `{timestamp, zones:[{type:OB|FVG, direction, timeframe:"H1", top, bottom, status:"unmitigated", candle_time} | {type:SWEEP, direction:high|low, level, candle_time}]}`. H1 only; MT5 H1 fetched via `mt5_safe` (Rule 23), broker-time→UTC (−3h). Tests: `test_flo455_ict.py` (extraction/filtering/JSON via mocked smc — the BETA detector itself must be **validated visually on the chart**, per the ticket). **Phase 2 (NOT done):** `ICTZoneDrawer.mq5` to read `ict_zones.json` every 10s and draw OB/FVG rectangles + sweep dashed lines (mirror `SRZoneDrawer.mq5`); needs operator MT5 compile/attach. **Phase 3 (NOT done):** `get_ict_zones` tool for Floki. Linear: FLO-455.
