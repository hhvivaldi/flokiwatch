...
| `last_analysis.data_needs` | object \| null | `ai_agent.py` → `main.py` → `state_writer.py` | FLO-302 / FLO-306: Floki's structured self-assessment. Shape: `{not_called: string[], unavailable: string[], timeframes_skipped: string[] (D1\|H4\|H1\|M15\|M5\|M1), biggest_obstacle: string, suggestions: string[], tool_errors: string[], assessment: string}`. **`not_called`** = tools/data Floki had access to but skipped this cycle (chronic skips signal blind spots). **`unavailable`** = genuinely missing/errored/stale data (chronic items signal broken infrastructure). Back-compat: legacy `missing_data` key still accepted and wrapped into `not_called`; legacy plain-string payloads wrapped as `{assessment: <str>, others empty}`. Mirrored to Discord `#data_needs` when any of not_called/unavailable/biggest_obstacle/suggestions/tool_errors is non-empty; drift detection runs independently on `not_called` and `unavailable` (3+ consecutive cycles → labeled drift warning). Never affects decisions. |
