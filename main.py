...
class TradingBot:
    ...
    def _call_agent_proactive_snapshot(self, trigger_type: str, snapshot_time_iso: str, agent_data: dict, df, trigger_data: Optional[dict] = None):
        ...
        try:
            ...
            # FLO-434 (CEO directive 2026-05-17): Rex Bull/Bear debate +
            # Research Manager removed from the Floki cycle. Floki (Opus)
            # is the sole decisor; weaker-model intermediaries (GPT-5-mini
            # for Rex, Gemini 3 Flash for RM) added noise. PLAN-20260514-009
            # forensic confirmed Floki overrode the RM verdict anyway.
            # rex_validator.py and research_manager.py are retained on disk
            # but no longer called by the cycle. Floki can still pull the
            # proactive Rex monitor via get_rex_monitor (independent scan
            # every 30 min, observational findings).
